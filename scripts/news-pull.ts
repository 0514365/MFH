...
async function main() {
  ...
  const guide = [
    ...
    '   → ★오래된 기사로 칸을 채우지 말 것. 새 소식이 없는 분야는 비우는 것이 정답이다.',
    '',
    '[절차]',
    ...
    `- 정치(politics) ★당일필수: "Honduras política hoy ${today}" 의회·정부·선거·정책. 최근 72시간 내 우선.`,
    '- 경제(economy) ★당일필수: "Honduras economía hoy" 환율·물가·고용·투자·송금(remesas). 최근 72시간 내 우선.',
    '- 사회(society) ★당일필수: "Honduras sucesos seguridad hoy" 치안·사건·보건·교육·이민. 최근 72시간 내 우선.',
    '- 문화(culture) 완화: "Honduras cultura noticias" 종교·축제·스포츠·교회. 최근 7일 내 허용, 없으면 생략.',
    ...
    '- ★안전(safety): OSAC/US Embassy/ReliefWeb 에서 "신규·갱신된" 여행경보·치안경보·기상/재난 alert 가 있을 때만 highlights(tag "안전"). 상시 동일 경보의 단순 반복은 제외.',
    '',
    '[정리 규칙 — 반드시 준수]',
    '- ★최신성 차등(이 페이지의 핵심):',
    '   · 정치·경제·사회 = "그날의 최신"을 반영한다. 발행일이 최근 72시간(불가피하면 7일) 이내인 소식만 넣고, 그보다 오래된 기사는 넣지 않는다(이미 지난 동향에 보관됨).',
    '   · 문화 = 새 소식이 적으면 최근 7일 내 1~2건, 없으면 비운다(빈 배열).',
    '   · San Pedro Sula·한인·안전 = ★새 소식이 없으면 비운다(highlights 에서 제외). 오래된 기사로 억지로 채우지 말 것. 한인은 본래 소식 빈도가 매우 낮다(현지 한인 약 280명, 한인회 휴면) — 없는 게 정상이다.',
    ...
    '- 분야별 2~3건(빈 분야는 0건 허용). 각 항목: title(한국어 한 줄) · body(2~4문장 한국어) · source(매체명) · url(기사 원문 https://…) · published_at("YYYY-MM-DD"). ★url 은 앱에서 출처 클릭에 쓰이니 가능하면 반드시 채운다.',
    ...
    '}',
    '※ highlights 는 새 소식이 있는 tag 만 넣는다. 셋 다 없으면 빈 배열 []. (San Pedro Sula·한인·안전 각각 선택)',
    '',
    '═══════════════════════ 최근 저장된 헤드라인 (중복 회피용) ═══════════════════════',
    recentBlock,
  ].join('\n')
