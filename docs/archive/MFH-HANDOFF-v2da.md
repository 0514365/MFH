...
> 이전: `v2cz`(메일 서명 3종). 이번 세션(2026-09-03~04): **성경통독(Bible in a Year) 모듈 신설** — 계획 수립·일정 자동 배분·읽음 체크·통독 방법·한 줄 은혜·기도제목 연동·홈 카드·이전 기록 수정. 커밋 4건(`b672b5e`→`a49a09c`→`4913d81`→`a0e4cba`) 모두 push·배포 완료. 앱 버전 3.4.0 유지(버전 제안은 우진이 "버전" 꺼낼 때).

---

## 현재 위치 (한 줄)
**성경통독 모듈 배포·실기기 검증 완료(patch103·104 적용됨).** 마지막 커밋 `a0e4cba`(최초 기록 보존 + 이전 기록 「수정」→「수정 완료」 흐름)는 push 됐고 우진 확인 대기. 다음 = 실사용 피드백 반영 또는 이월 과제.
...
1. **우진 확인 대기**(`a0e4cba` 배포분): 체크 해제→재체크 시 최초 시각 유지 / 이전 기록 행 탭 → 요약 → 「수정」 → 「수정 완료」 저장·「취소」 복원.
2. 실사용 후 조정 후보: 밀린 분량 **재분배**(남은 날에 미읽은 장 재배분), 구약·신약 병행 읽기(README 10절), 완독 시 `completed_at` 자동 기록(현재는 진행률 100% 판정만), 통독 통계(주간 읽은 장·평균 소요 분).
