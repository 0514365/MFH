// MFH-QT-VIEW-V1
// 일일 QT 1일치 렌더(본문 메타 + 핵심절 + 묵상 + 적용 + 기도 + 출처 푸터). /qt 가 사용. 서버 컴포넌트(표시 전용).
// 저작권: 성경 본문 전체·성서유니온 묵상은 저장/표시하지 않는다. 본문 메타(책·장절·찬송) + 핵심절(개역개정 짧은 인용) + 자체 묵상·적용·기도만.
// 폰트: BriefingView 와 동일 — 본문 16~17px 기준, 제목은 font-display.
import type { ReactNode } from 'react'

export type QtPassage = { book?: string | null; book_en?: string | null; range?: string | null; hymn?: string | null; source_url?: string | null }
...
export const QT_SELECT = 'id,qt_date,passage,key_verse,meditation,application,prayer_points,created_at'
...
export default function QtView({ row, headerAction }: { row: QtRow; headerAction?: ReactNode }) {
  const p = row.passage ?? {}
  ...
  return (
    <div className="space-y-6">
      ...
      <header>
        ...
        <div className="mt-0.5 flex items-end justify-between gap-3">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-faint">매일성경</span>
          </div>
          {headerAction && <div className="shrink-0 pb-0.5">{headerAction}</div>}
        </div>
      </header>

      {/* 본문 — 책·장절·찬송 + 성서유니온 원문 링크 */}
      <section className="rounded-2xl border border-primary/15 bg-primarySoft p-5">
        ...
        <p className="mt-2 font-display text-2xl font-extrabold leading-tight text-ink">
          {book} {range}
        </p>
        {bookEn && <p className="mt-0.5 text-sm text-muted">{bookEn}</p>}
        {hymn && <p className="mt-2 text-sm text-muted">찬송 · {hymn}</p>}
        {sourceUrl && (
          <a
            href={sourceUrl}
            target="_blank"
      ...
      <p className="pt-1 text-center text-xs leading-relaxed text-faint">
        ...
        <br />
        묵상·적용·기도는 우리 일지·사역 기록을 바탕으로 작성되었습니다.
      </p>
    </div>
  )
}
