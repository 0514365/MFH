...
- **워크트리 세션 통합 규칙**: 세션이 `.claude/worktrees/`(브랜치 `claude/...`)에서 작업 중일 때 우진이 "푸시" 라고 하면 한 번에 처리한다 — ① 브랜치 커밋 확인 → ② 메인 repo 폴더(`git -C <repo루트>`)에서 main 에 머지 → ③ origin/main 푸시 → ④ 머지된 원격 `claude/...` 브랜치 삭제. (main 에 미커밋 변경이 있어도 이번 파일과 겹치지 않으면 머지 진행, 겹치면 멈추고 묻는다.)
...
2. 사역 기도제목은 **사역 블록마다 1~2개**로 압축하고, **마무리 요약 카드는 사역 항목별로 한 줄씩** 정리한다(사역이 4~5개면 요약도 4~5줄 — 억지로 1~2개로 합치지 않는다). 요약 카드 항목은 **원숫자가 아니라 원형 불릿**. *(2026-06·07호 실물 기준으로 2026-08 정정)*
...
**앱 버전(SemVer)**: 현재 **3.0.0** — "4모듈 기록 앱(~2.x)" 에서 **선교편지 5-에이전트 + AI 자동화 플랫폼**으로 정체성이 도약한 MAJOR 기점. 단일 출처(SoT)는 `package.json` 의 `version` — 홈 footer(`app/page.tsx`)가 이 값을 **자동 표기**하므로 버전을 올리면 화면도 자동 반영된다(**footer 버전 하드코딩 금지** — 새 화면에 버전을 노출할 때도 반드시 `package.json` 의 `version` 을 읽어 쓴다). 핸드오프 코드네임(`v2bX`)은 세션 추적용으로 별개 유지.
- **버전 증가 제안 규칙(필수)**: **우진이 "버전" 을 꺼낼 때만** Claude 가 직전 버전 이후 **누적 변경점을 검토해** 등급과 다음 버전을 **한 줄로 제안**하고, 승인 후 `package.json` 에 반영한다(**커밋마다 자동 제안하지 않는다**). PATCH(`3.0.x`)=버그 수정·소소한 텍스트/스타일, MINOR(`3.x.0`)=하위호환 기능 추가(새 화면·모듈·스킬), MAJOR(`x.0.0`)=제품 축·정체성 도약·호환 깨는 대변경.

**현재 위치**: 포트폴리오 모듈 진행 중. 최신 상태는 `docs/` 의 `MFH-HANDOFF-*.md`(**최신본 1개만 유지**)를 읽고 판단한다. 세션 종료 시 새 핸드오프를 작성하고 **직전 버전은 `docs/archive/` 로 이동**한다(이전 버전 전부 보관).

## 9. 세션 시작 시 Claude 가 할 일

1. `docs/` 의 핸드오프(`MFH-HANDOFF-*.md` — 최신본만 있음, 이전 버전은 `docs/archive/`) + `MFH-CONTEXT.md` 를 읽는다.
