...
- **버전 증가 제안 규칙(필수)**: 의미 단위 작업(커밋 단위)을 마칠 때마다 Claude 가 이번 변경 등급과 다음 버전을 **한 줄로 제안**하고, 우진 승인 후 `package.json` 에 반영한다. PATCH(`3.0.x`)=버그 수정·소소한 텍스트/스타일, MINOR(`3.x.0`)=하위호환 기능 추가(새 화면·모듈·스킬), MAJOR(`x.0.0`)=제품 축·정체성 도약·호환 깨는 대변경.
