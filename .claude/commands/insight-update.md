...
2. **분석·작성** — 작업지시서를 **그대로 충실히 따라** 7개 도메인(overall · journal · project · task · prayer · fruit · letter) 인사이트를 각각 작성한다.
...
3. **결과 저장 파일 쓰기** — 7개 블록을 모두 이어 `insights-archive/_result.md` 에 Write 한다(이 폴더는 gitignore).
...
- **letter(편지 방향 개요)**: 작업지시서 하단의 "편지 재료 — 최근 인사이트 + 내 피드백 신호"(별점·메모·편지에담기·보관)를 **우선 반영**해 방향을 잡고, "온두라스 소식"은 **WebSearch 로 최신 현지 뉴스**(기상·경제·사회)를 확인해 정당·인물 거명 없이 중립적으로 녹인다.
