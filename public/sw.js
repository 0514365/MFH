...
// V5: 오프라인 지원(2단계 읽기). install 시 정적 폴백 /offline.html 을 precache 하고, fetch 에서
//     ① navigation(페이지 이동) = network-first → 실패 시 캐시된 /offline.html 폴백(흰화면 방지)
//     ② 정적 자산(_next/static·아이콘·폰트) = cache-first(런타임 적재, 해시 파일명이라 안전)
//     ③ /api·비GET·교차출처 = 미관여(네트워크 그대로).
//   ※ 폴백은 React 라우트가 아니라 자기완결 정적 HTML(/offline.html) 이다. React 라우트는
//     JS 청크 의존 때문에 미방문 시 오프라인에서 청크를 못 받아 깨진다(client-side exception).
//     정적 HTML 은 의존이 없어 이 파일 하나만 precache 하면 회선이 끊겨도 100% 뜬다.
//   ※ V3 회귀 주의: navigate 요청은 반드시 fetch(request) 를 "그대로" 호출한다(init 옵션 금지).
...
self.addEventListener('fetch', (event) => {
  ...
  // ① 페이지 이동: network-first → 실패하면 캐시된 정적 /offline.html 로 폴백.
  ...
  // ② 정적 자산: cache-first(해시 파일명이라 불변 — 한 번 받으면 오프라인에서도 앱이 뜬다).
  const isStatic =
