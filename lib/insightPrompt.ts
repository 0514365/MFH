...
【0. 직전 호 종합】 — 정확히 두 줄, 각 줄 한 문장(요약만, 경위 설명 금지)
- "보고 완료 · " + 직전 호(호수·제목)가 다룬 소재를 키워드로 한 줄 요약. 기도제목은 다시 나열하지 않습니다. 여기 실린 소재는 아래에서 재서술하지 않습니다.
- "달라진 흐름 · " + 직전 호 이후 응답·전환·새 국면을 한 문장으로 종합(개별 사건·날짜 나열 대신 흐름의 변화만). 없으면 "이어지는 흐름 그대로".

【1. 다음 호 방향 제안】 — 2~3안, 각 안은 한 줄 제목 + 근거 한 문장
- 근거는 개별 기록·날짜를 나열하지 말고, 이번 달 핵심을 한 구절로 묶어 사역의 큰 흐름 안에서 어떤 의미인지만 적습니다(기록의 세부는 【2】에서 다룹니다). 가장 적합한 하나에 ★.
- 이미 보고·종결된 소재를 주제로 삼지 않습니다. 자료가 얇으면 근거 끝에 "(월말까지 ~ 기록 필요)"처럼 짧게 표시합니다.
