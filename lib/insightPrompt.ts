...
- 일상에서 잘 쓰지 않는 번역투·상투적 비유를 피하고 쉬운 말로 적습니다(예: '한 장으로 정리'·'한곳에 모아'·'한 걸음씩' 대신, 무엇을 어떻게 하는지 구체적으로). 같은 비유를 여러 항목에서 반복하지 않습니다.`
...
export const LENS_FOCUS: Record<LensKey, string> = {
  ...
  letter:
    '월간 기도편지 렌즈는 "전체 사역의 흐름과 맥락 속에서, 이번 달에 이루어진 일의 핵심은 무엇이고 다음 호 편지에 무엇을 담을까"에 답합니다. 세 가지를 함께 놓고 분석합니다. ① 90일 분석 데이터 = 사역의 큰 흐름과 맥락(어디서 와서 어디로 가는지). ② 「편지 기준점 — 직전 호」(호수·제목·요약) = 이미 후원자에게 보고했거나 종결된 사항의 제외 기준이자, 그 뒤 흐름이 바뀐 것(응답·전환·중단·새 국면)을 알아보는 비교 기준. ③ 「직전 호 이후 기록」(직전 호 자료 기간 종료일 다음날~오늘의 일지·프로젝트·할 일·QT 묵상·사진 캡션·온두라스 브리핑) = 이번 호에 담을 새 재료. 직전 호에 실린 소재는 다시 서술하지 않되, 그것이 이번 달에 새 국면으로 이어졌다면 그 변화만 담습니다. 온두라스 소식은 브리핑에 기록된 것만 정당·인물 거명 없이 중립적으로 사역 환경과 접목합니다(기록에 없는 수치·사건 금지). 완성된 편지가 아니라, 우진이 살을 붙일 방향과 뼈대, 그리고 편지화에 부족한 자료 목록을 줍니다.',
  project_assist:
  ...
  supporter_care:
    '후원자 관계관리는 "누구에게 어떤 마음을 전할까"에 답합니다. 헌금·관계 기록·기도제목을 종합해, 최근 연결이 활발한 후원자와 한동안 연락이 뜸한 후원자를 살피고, 감사·안부·심방의 다음 한 걸음을 제안합니다. 후원자를 관리 대상이 아니라 함께 사역하는 동역자로 존중하며 따뜻한 관계의 언어로 씁니다. 헌금 액수로 사람을 비교·평가하지 않습니다.',
}
...
- 감사의 언어로, 기록에 충실하게.`,
  letter: `[출력 형식 — 월간 기도편지: 직전 호 종합 → 다음 호 방향 제안 → 초안 개요 → 자료 공백]
- 맨 위에 다음 호 번호 'MFH #YYMM'(작업지시서 「편지 기준점 — 직전 호」의 "다음 호 번호" 그대로).
- 시간순 사건 나열은 하지 않습니다. 사역의 흐름·맥락 속에서 이번 달의 핵심을 골라 편지에 담을 내용으로 정리하는 것이 목적입니다.

【0. 직전 호 종합】
- 직전 호(호수·제목)가 후원자에게 이미 보고한 소재와 기도제목을 요약문 기준으로 2~3줄. 이것들은 "보고 완료·종결"로 보고 아래에서 재서술하지 않습니다.
- 이어서 "그 뒤 달라진 흐름" 1~3줄: 직전 호 기도제목·소재 가운데 이번 달 응답·전환·중단·새 국면으로 이어진 것만 짚습니다(날짜 포함). 없으면 "이어지는 흐름 그대로"라고 적습니다.

【1. 다음 호 방향 제안】
- 위 종합을 반영한 상태에서, 90일 흐름과 직전 호 이후 기록을 함께 놓고 이번 달을 관통하는 주제 앵글을 2~3개 제안합니다. 각 방향 = 한 줄 제목 + 근거 2~3문장(이번 달 어떤 기록(날짜)이 핵심인지, 그것이 사역의 큰 흐름 안에서 어떤 의미인지). 가장 적합한 하나에 ★.
- 이미 보고·종결된 소재를 주제로 삼지 않습니다. 자료가 아직 얇으면 "지금은 ~ 방향이 보이나 월말까지 ~ 기록이 더 필요" 식으로 솔직히 적습니다.

【2. 추천 방향 초안 개요】
추천 방향을 3단 구조의 '개요'(완성 문장이 아니라 뼈대·소재)로 제시합니다. 각 항목은 이번 달 핵심 사건과 그 맥락(왜 중요한지)을 함께 담습니다.
**1. 온두라스 소식** — 작업지시서에 수록된 온두라스 동향 브리핑 중 사역 환경과 닿는 1~2가지만 중립적으로 접목. 정당·인물·진영 거명 절대 금지. 정치 사안은 '사회 안정·치안·회복을 위한 중립 기도'로만 녹입니다. 브리핑에 없는 수치·사건은 쓰지 않습니다.
**2. 사역 소식** — 이번 달 핵심 사역 3~5개(대시 불릿). 각 불릿은 날짜·사건·맥락(직전 호에서 어떻게 이어졌는지 필요할 때 한 구절). 직전 호 소재는 새 국면이 있을 때만 그 변화만 씁니다.
**3. 선교사 가정** — 가족 근황·평강·내면의 고백(대시 불릿). 묵상(QT·일지)에서 이번 달을 요약하는 말씀 줄기가 있으면 한 줄로.
- 마지막에 "기도제목" — 온두라스(중립)·사역 1~2개·가정 순으로 "- 라벨 · 본문" 대시 불릿. 라벨은 온두라스/사역/가정. 직전 호 기도제목과 같은 문장을 반복하지 않고 이번 달 국면에 맞춰 새로 씁니다.

【3. 자료 공백】
- 편지화에 부족한 것을 2~5개 대시 불릿으로: 확인이 필요한 사실·날짜, 사진이 없는 사역, 아직 결과가 기록되지 않은 예정 행사 등. 우진에게 물을 질문 형태로 짧게.
- 개요·불릿 모두 핵심 소재·문장 씨앗 위주. 기록에 없는 사실은 지어내지 않습니다.`,
  project_assist: `[출력 형식 — 프로젝트 비서]
- 3~5개 항목. 각 항목은 두 줄입니다: 첫 줄은 **굵은 소제목**(프로젝트·주제 이름, 한 줄로 짧게), 다음 줄은 지금 손댈 다음 행동 한 줄.
- 소제목 줄에는 \`**…**\` 만 단독으로 둡니다(소제목 뒤에 본문을 붙이지 않음). 항목 사이에는 빈 줄을 넣지 않고 줄바꿈만 합니다.
- 본문은 간결한 실무형 한 문장. 군더더기·수식어를 빼고 핵심 동작만 남기며, 명사형 종결('…정리', '…확정')도 좋습니다. 필요하면 마감·중요도를 괄호로 짧게.
- 정체된 것·마감 가까운 것을 위에 둡니다. 기록에 없는 일은 지어내지 않습니다.
- 맨 끝에 빈 줄 하나를 두고 "**이번 주 우선** · <프로젝트 한 줄>"을 적습니다.`,
  task_assist: `[출력 형식 — 할 일 비서]
- 3~5개 항목. 각 항목은 두 줄입니다: 첫 줄은 **굵은 소제목**(할 일·주제 이름, 한 줄로 짧게), 다음 줄은 오늘·이번 주에 할 다음 행동 한 줄.
- 소제목 줄에는 \`**…**\` 만 단독으로 둡니다. 항목 사이에는 빈 줄을 넣지 않고 줄바꿈만 합니다.
- 본문은 간결한 실무형 한 문장. 군더더기를 빼고 핵심 동작만 남기며, 명사형 종결도 좋습니다. 필요하면 마감·중요도를 괄호로 짧게.
- 마감 임박·고중요도·정체 순으로 위에서부터 배치합니다. 기록에 없는 일은 지어내지 않습니다.
- 맨 끝에 빈 줄 하나를 두고 "**오늘 딱 하나** · <할 일 한 줄>"을 적습니다.`,
  supporter_care: `[출력 형식 — 후원자 관계관리]
네 부분으로 나눠 작성합니다(각 소제목은 굵게).
**1. 관계 현황** — 최근 연결이 활발한 후원자, 한동안 연락이 뜸한 후원자를 각각 1~3명 짚습니다(데이터 근거로만).
**2. 다음 액션** — 지금 할 구체적 행동 3~5개. 각 항목: \`**후원자/주제**\` 한 줄 + 다음 줄에 연락·감사·심방 등 한 줄 행동.
**3. 함께 기도할 제목** — 후원자들의 기도제목을 1~3개로 모읍니다(기도 3원칙 준수, 개인 식별정보는 절제).
**4. 감사·안부 메시지 초안** — 한 후원자에게 보낼 짧은 메시지 예시 1개(3~5문장, 따뜻하고 담백하게).
- 기록에 없는 사실은 지어내지 않습니다. 후원자가 적거나 기록이 부족하면 솔직히 적습니다.`,
}
...
export function buildBundleInstruction(domains: InsightDomain[]): string {
  ...
  return [
    ...
    hasLetter
      ? 'LENS: letter 는 맨 마지막에 작성합니다. 아래 90일 분석 데이터는 사역의 흐름·맥락으로 보고, 작업지시서의 「편지 기준점 — 직전 호」는 이미 보고·종결된 것의 제외 기준으로, 「직전 호 이후 기록」은 이번 호에 담을 새 재료로 삼아, 세 가지를 함께 반영한 상태에서 다음 호 방향과 개요를 잡아 주세요(위에서 작성한 prayer·fruit 은 참고만). letter 블록의 PERIOD 는 「편지 기준점」에 적힌 기간을 씁니다.'
      : '',
    '',
    blocks.join('\n\n'),
  ].join('\n')
